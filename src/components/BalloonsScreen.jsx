...
export default function BalloonsScreen({ onNext }) {
  ...
  const balloons = [{ text: "I", delay: 3, color: "fill-pink-500/50" }, { text: "Love", delay: 4.5, color: "fill-rose-500/50" }, { text: "You", delay: 6, color: "fill-purple-500/50" }]
  ...
  const BalloonComponent = ({ balloon }) => (
    <motion.div
      ...
      transition={{
        y: { delay: balloon.delay, duration: 2, ease: "easeOut" },
    ...
    >
      <div className="relative">
        ...
        {/* balloon circle */}
        <div className="relative">
          <div className="absolute inset-0 bg-pink-500/10 blur-2xl" />
          <motion.div
          ...
          >
            ...
            <span className="text-white font-bold text-2xl md:text-3xl z-10 drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]">{balloon.text}</span>
          </motion.div>
        </div>
      </div>
    </motion.div>
